            ...
            for group in product_groups:
                ...
                for _, shp in group_shipments.iterrows():
                    ...
                    individual_results.append({
                        ...
                        # ADDED: passthrough of whatever caller-supplied row
                        # identifier came in on the input row (blank if none
                        # was given) -- shipments get grouped/reordered above,
                        # so this is what lets the API layer match each output
                        # row back to the request that produced it.
                        "source_row_id": shp.get("source_row_id", ""),
...
def register_vehicle(vehicles_df: pd.DataFrame, *, vehicle_no: str, mode: str, size_class: str = "",
                      refrigerated="N", weight_capacity_kg: float = 0.0, volume_capacity_m3: float = 0.0,
                      temp_control_min_C="", temp_control_max_C="", price_per_kg_INR: float = 0.0,
                      fixed_cost_INR: float = 0.0, avg_speed_kmph: float = 40.0, driver_name: str = "",
                      driver_contact: str = "", origin: str = "", destination: str = "",
                      remaining_capacity_kg=None):
    """ADDED (non-interactive): same row shape and validation as
    register_vehicle_interactively() below, but takes plain arguments instead
    of input() -- this is what the FastAPI layer calls for POST
    /api/vehicles/register. register_vehicle_interactively() now delegates
    here too, so the CLI keeps behaving exactly as before.

    remaining_capacity_kg lets a vehicle be registered already part-loaded
    (e.g. the fleet form's "Current Utilized Load" field) instead of always
    starting full -- needed so half-full vehicles can correctly be matched
    only to same-route orders per the fleet-matching rule.

    Returns (updated_vehicles_df, new_row_dict_or_None, error_message_or_None).
    """
    vehicle_no = str(vehicle_no).strip()
    if vehicle_no and vehicle_no in vehicles_df["vehicle_no"].astype(str).values:
        return vehicles_df, None, f"Vehicle number '{vehicle_no}' is already registered."

    mode = str(mode).strip().capitalize()
    if mode not in ("Road", "Rail"):
        return vehicles_df, None, f"Mode must be 'Road' or 'Rail' (got '{mode}')."

    refrigerated = "Y" if str(refrigerated).strip().upper() in ("Y", "YES", "TRUE", "1") else "N"
    if refrigerated == "N":
        temp_control_min_C, temp_control_max_C = "", ""

    try:
        weight_capacity_kg = float(weight_capacity_kg)
    except (TypeError, ValueError):
        return vehicles_df, None, "Weight capacity must be a number."

    if remaining_capacity_kg in (None, ""):
        remaining_capacity_kg = weight_capacity_kg
    try:
        remaining_capacity_kg = max(0.0, min(float(remaining_capacity_kg), weight_capacity_kg))
    except (TypeError, ValueError):
        remaining_capacity_kg = weight_capacity_kg
    ...
    new_row = {
        ...
        "volume_capacity_m3": float(volume_capacity_m3) if str(volume_capacity_m3).strip() not in ("", "None") else 0.0,
        "temp_control_min_C": temp_control_min_C,
        "temp_control_max_C": temp_control_max_C,
        "price_per_kg_INR": float(price_per_kg_INR) if str(price_per_kg_INR).strip() not in ("", "None") else 0.0,
        "fixed_cost_INR": float(fixed_cost_INR) if str(fixed_cost_INR).strip() not in ("", "None") else 0.0,
        "avg_speed_kmph": float(avg_speed_kmph) if avg_speed_kmph else 40.0,
...
def register_vehicle_interactively(vehicles_df: pd.DataFrame) -> pd.DataFrame:
    ...
    CHANGED: the row-building/validation now lives in register_vehicle()
    above so it can be reused from the API -- this function is just the
    stdin prompt wrapper around it, prompts and messages unchanged."""
    ...
    updated, new_row, error = register_vehicle(
        vehicles_df, vehicle_no=vehicle_no, mode=mode, size_class=size_class, refrigerated=refrigerated,
        weight_capacity_kg=weight_capacity_kg, volume_capacity_m3=volume_capacity_m3,
        temp_control_min_C=temp_min, temp_control_max_C=temp_max, price_per_kg_INR=price_per_kg_INR,
        fixed_cost_INR=fixed_cost_INR, avg_speed_kmph=avg_speed_kmph, driver_name=driver_name,
        driver_contact=driver_contact, origin=origin, destination=destination,
    )
    if error:
        print(f"  [Error] {error} Cancelling.")
        return vehicles_df

    print(f"\nVehicle {vehicle_no} registered with {weight_capacity_kg} kg capacity, based at {origin}.\n")
    return updated

def free_up_vehicle(vehicles_df: pd.DataFrame, vehicle_no: str):
    """ADDED (non-interactive): same effect as free_up_vehicle_interactively()
    below (resets one vehicle's remaining_capacity_kg to full), callable from
    the API. Returns (updated_vehicles_df, restored_capacity_or_None,
    error_message_or_None)."""
    vehicle_no = str(vehicle_no).strip()
    mask = vehicles_df["vehicle_no"].astype(str) == vehicle_no
    if not mask.any():
        return vehicles_df, None, f"Vehicle '{vehicle_no}' not found in the fleet. No changes made."

    updated = vehicles_df.copy()
    full_capacity = float(updated.loc[mask, "weight_capacity_kg"].iloc[0])
    updated.loc[mask, "remaining_capacity_kg"] = full_capacity
    return updated, full_capacity, None
...
def free_up_vehicle_interactively(vehicles_df: pd.DataFrame) -> pd.DataFrame:
    ...
    CHANGED: delegates to free_up_vehicle() above; prompts and messages
    unchanged."""
    print("\n--- Free Up Vehicle (Trip Completed) ---")
    vehicle_no = input("Vehicle number to free up: ").strip()

    updated, restored, error = free_up_vehicle(vehicles_df, vehicle_no)
    if error:
        print(f"  [Error] {error}\n")
        return vehicles_df

    print(f"\n{vehicle_no} is now free with full capacity ({restored} kg) restored.\n")
    return updated
