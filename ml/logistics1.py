...
def find_candidate_routes(origin_city: str, destination_city: str, weight_kg: float, refrigerated: bool,
    ...
    try:
        ...
        if c["route"] not in [x["route"] for x in candidates]:
            candidates.append(c)
    except (nx.NetworkXNoPath, nx.NodeNotFound):
        pass
                ...
                for _, shp in group_shipments.iterrows():
                    ...
                    individual_results.append({
                        ...
                        "route_selection_explanation": f"Selected {best['modes_used'][0] if len(best['modes_used'])==1 else 'Multimodal'} because its risk-adjusted cost was lowest. "
                                                        f"Gross cost INR {best['gross_cost_INR']:.2f}; Kisan Rail subsidy INR {best['kisan_rail_subsidy_INR']:.2f}; "
                                                        f"net cost INR {best['total_cost_INR']:.2f}; expected transit {best['total_expected_time_hr']:.2f} hr.",
...
def print_shipment_plan(output: pd.DataFrame, audience: str = "farmer"):
    ...
    if "modes_used" in output.columns and "group_id" in output.columns:
        multimodal_groups = output[output["modes_used"].apply(
            lambda m: len(set(str(m).replace('[','').replace(']','').replace("'",'').replace('"','').split(','))) > 1
        )]["group_id"].nunique()
    else:
        multimodal_groups = 0

    print(f"\n----- SUMMARY -----")
    print(f"Farmers pooled: 1")
    ...
    print(f"Groups with High/Critical spoilage risk: 0 / {pooled_groups}\n")
